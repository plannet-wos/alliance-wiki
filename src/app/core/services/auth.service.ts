 ...
 * Replaces AdminSessionService's two sessionStorage booleans + article.service.ts's
 * authenticate()/authenticateSuperAdmin() write-trick — see the multi-state rollout plan for
 * why a real Firebase Auth identity is what makes this guard trustworthy at all (it wasn't
 * before: wiki_articles writes stay open regardless, by design, so this has always been a
 * UX nicety rather than real enforcement — see wiki-admin.guard.ts).
 */
...
export class AuthService {
  ...
  readonly isSuperAdmin = computed(() => this.isActive() && this.rank() === RANK.SUPERADMIN);
   ...
   * checking isAuthenticated()/isActive()/rank()/canAdminister(): those signals read as
   * "signed out, no account" for a beat after every auth-state change (a fresh page load
   * restoring a persisted session is itself async; signing in kicks off a new onSnapshot that
   * hasn't fired yet either), and a guard reading them synchronously in that window wrongly
   * bounces a real, signed-in user back to /login. Mirrors plannet-wos's auth.service.ts —
   * see its fuller doc comment. Safe to call from anywhere, any number of times — each call
   * just awaits whichever settle-gate is current.
   */
  ...
  /** True if the current account can administer this specific alliance's wiki (superadmin, or that alliance's own R4/R5). */
